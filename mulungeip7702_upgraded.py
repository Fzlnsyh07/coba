...
def build_calls(w3: Web3, victim: str, kernel_style: bool = False) -> list[dict]:
    """
    kernel_style=True  → pakai struct Execution {target, value, data} (untuk Kernel)
    kernel_style=False → pakai Multicall3 Call3Value {target, allowFailure, value, callData}
    """
    calls = []
    menu = {
...
def main():
    ...
    # Deteksi apakah pakai Kernel style (execute / executeBatch)
    kernel_style = any(x in batch_method.lower() for x in ["execute", "batch"])
    if kernel_style:
        print("  ℹ  Terdeteksi Kernel-style (execute/executeBatch) → pakai struct Execution {target, value, data}")

    calls = build_calls(w3, victim, kernel_style=kernel_style)
    if not calls:
        print("Tidak ada call. Exit."); sys.exit(0)

    total_value = sum(c["value"] for c in calls)

    # Encode dengan struct yang sesuai
    try:
        dc = w3.eth.contract(address=delegate, abi=delegate_abi)

        if kernel_style:
            # Kernel Execution struct: (target, value, data)
            tx_data = dc.encode_abi(
                abi_element_identifier=batch_method,
                args=[[ (c["target"], c["value"], c["data"]) for c in calls ]]
            )
        else:
            # Multicall3 style
            tx_data = dc.encode_abi(
